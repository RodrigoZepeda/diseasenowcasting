...
> Here we give an example of a continuous SIR ordinary differential
> equation with a [4th-order Runge–Kutta
> (RK4)](https://en.wikipedia.org/wiki/Runge%E2%80%93Kutta_methods)
> scheme and several sub-steps per day.
...
and the daily incidence is the number of new infections on that day —
*i.e.* the drop in `S`. We advance the state with `substeps` (RK4 steps
of size `dt = 1/substeps` per day) and accumulate the new infections.
...
sir_ode_fn <- function(theta) {
  ...
  #RK-4 hyperparameters
  substeps <- 4L      # 4 RK4 steps per day
  dt       <- 1 / substeps              
  ...
  #Loop throughout each date
  for (t in seq_len(max_time)) {
    new_today <- 0
    #RK-4 steps for each day
