...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Current number of cases for a certain disease (left) vs the eventual number of registered cases for that disease (right)", out.width = '75%'}
set.seed(24759)
library(dplyr)
library(ggplot2)
library(ggtext)
...
plt2 <- ggplot(cases) +
  ...
  labs(
    x = "Epidemiological week",
    y = "Cases",
    title = "Final number of cases per week"
  ) +
  theme_classic() +
  theme(plot.title = element_markdown()) +
  scale_x_continuous(breaks = 1:11)
...
Data for the `nowcasting` function should be either individual-level data or line-list data. In any case at least two columns are required: the `true_date` and the `report_date`. 
...
### Preprocess for nowcast

Once you have acquired your data you'll have to preprocess it for the `nowcast` function. Preprocessing involves transforming the data into line-list data and adding three columns into your dataset:

* `n` The number of cases with those characteristics

* `.tval` The `true_date` but transformed into integers with `min(true_date) = 1`.

* `.delay` The time difference between the `true_date` and the `report_date`. 

The `preprocess_for_nowcast` function helps you transform your data. 
