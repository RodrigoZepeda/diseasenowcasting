...
Once the data is a `tbl_now`, a single call to `nowcast()` does the rest
(the nowcast is automatically stratified by the strata declared above):
...
arrives, `update()` re-scores every incoming report against the
previously fitted delay distribution and **warns** about anything that
seems implausibly delayed. The flagged reports are available via
`extreme_values()`:
