# =============================================================================
# 39 -- THE ZTNB VERSION OF THE p-FREE MODEL, WITH MEAN PRESERVATION ACTUALLY
#       HOLDING.  Supersedes script 38.
#
# Script 38 intended E[Delta] = alpha - omega but did not achieve it: it passed
# (alpha+omega)/pi to dnbinom(mu=), i.e. as the PARENT mean, whereas a
# zero-truncated NB has mean parent/(1-P0) > parent.  At the fitted size = 0.015
# the realised mean was inflated 8x to 37x, so E[C_t(d)] was NOT mu_t q_C(d) and
# the tex identification argument did not transfer.  FINDINGS U carries the
# correction.
#
# Fixing it by inverting Psi(m) = m/(1-P0(m)) per cell would need a root find
# inside the likelihood (~900 cells x thousands of evaluations).  Instead
# reparameterise so no inversion is needed: make the PARENT mean the free
# quantity and DERIVE the hurdle probability.
#
#     log m_t^d = c0 + c1 log(1+d) + c2 1{previous update non-null}
#     nu_t^d    = Psi(m_t^d) = m/(1-P0(m))          closed form, = E[M]
#     pi_t^d    = (alpha_t^d + omega_t^d) / nu_t^d  derived, admissible iff <= 1
#
# Then E[Delta_t^d] = pi * E[M] * (2 theta - 1) = (alpha+omega)(alpha-omega)/(alpha+omega)
# = alpha - omega EXACTLY, by construction and with no numerical inversion.  The
# age and Markov terms simply move from logit(pi) onto log(m).  Admissibility
# pi <= 1 is the mechanistic statement that no more cohorts can move than there
# are expected report/retraction events; cells violating it are clamped and
# counted.
#
...
#
# Run: NORIG=12 NOT_CRAN=true Rscript devel/spa_diagnostics/39_meanpreserving.R
# =============================================================================
...
S0 <- c(-1, log(0.5), stats::qlogis(0.5), rep(0, 14), -1.0, 0.5, -0.3, log(0.2))
starts <- list(S0,
...
cat(sprintf("log m: %+.2f %+.2f log(1+d) %+.2f 1{prev} | NB size %.4f\n",
            P$c0, P$c1, P$c2, P$size))
Qf <- q_pairs(P$gD, P$hR, fl$a, fl$b)
mu_i <- mu[fl$grp]; al <- pmax(mu_i*Qf$qp, 1e-10); om <- pmax(mu_i*Qf$qm, 1e-10)
th_i <- al/(al+om); Di <- derive(P, fl$b, fl$pm, al+om)
pi_i <- Di$pi; mM <- Di$m; k <- which(fl$post)
cat(sprintf("admissibility pi <= 1: %d of %d post-first cells clamped\n",
            sum((al+om)[k]/pmax(Di$nu[k], 1+1e-12) > 1), length(k)))
# DIRECT CHECK that mean preservation now holds
Edelta <- pi_i * Di$nu * (2*th_i - 1)
cat(sprintf("mean preservation: max |E[Delta] - (alpha-omega)| over post-first = %.3e\n",
            max(abs(Edelta[k] - (al-om)[k]))))
...
sim_fwd <- function(C0, st0, mu_t, ages, P, n) {
  ...
  for (g in ages) {
    ...
    th_ <- a_/(a_+o_)
    D <- derive(P, g, st, a_ + o_)
    pi_ <- D$pi; m_ <- D$m
    mv <- stats::runif(n) < pi_
    if (any(mv)) {
      p0 <- stats::dnbinom(0, size = P$size, mu = m_[mv])
      M <- stats::qnbinom(stats::runif(sum(mv), p0, 1), size = P$size, mu = m_[mv])
...
saveRDS(R, sprintf("devel/spa_diagnostics/backtest_meanpres_%s.rds", gsub(" ","",STATE)))
cat(sprintf("\n=== %d triples ===\n", nrow(R)))
cat("\n--- WIS by horizon ---\n")
...
cat(sprintf("mean-preserving (39, this)    : WIS %.3f | skill %+.1f%%\n",
    mean(R$wis_hyb), 100*(1-mean(R$wis_hyb)/mean(R$wis_emp))))
cat(sprintf("script 38 (mean NOT preserved): WIS 1.702 | skill +22.4%%\n"))
