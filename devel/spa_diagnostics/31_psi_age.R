# =============================================================================
# 31 -- STAGE 1c: age-dependent revision asymmetry.
#
# Script 30's two revision scales fixed the zeros (609 predicted vs 620 observed,
# against 85 for the retention model) and bought +90 nats over one scale, but
# gates 2 and 3 still fail, and gate 3's SHAPE moved in an informative way:
#
#   one scale (29): deciles 0.076 0.068 ... 0.138 0.073  -- depleted at BOTH
#                   ends, i.e. the predictive was too wide in the body;
#   two scales(30): deciles 0.089 0.068 0.081 0.093 0.120 0.087 0.091 0.111
#                   0.122 0.138 -- now RISING toward 1.
#
# A PIT that rises toward 1 means observations sit in the upper tail of the
# predictive too often: the predictive is centred too LOW.  That is exactly what
# stage 0 table L.4 shows the model cannot yet do -- at age 1, 46.7% of cells
# move UP against 11.4% down, while by age 15 it is 1.2% up against 1.1% down.
# Movement is strongly positive when young and symmetric when old.
#
# The model cannot supply that drift at present:
#   * arrivals cannot, because g_D collapsed to P(0) = 0.998.  Giving g_D mass at
#     delay 1 would add alpha to EVERY age-1 cell, including the 42% that are
#     exactly zero, so the zeros gate forbids it;
#   * the revision cannot, because psi is a single constant.  Raising it adds
#     positive drift at every age, when the drift is concentrated at ages 1-2.
#
# So make the asymmetry age-dependent, which is one parameter:
#
#     psi(a) = exp(v0 + v1 log(1+a))
#
# psi > 1 tilts a revision upward, psi = 1 is symmetric.  Everything else is
# script 30 unchanged, so the change is attributable.
#
# Run: COLD=1 START=1 NOT_CRAN=true Rscript devel/spa_diagnostics/31_psi_age.R
# =============================================================================
...
# theta = (dmu, log dsig, p0, p1, logit q, log sigma_s, log sigma_1, v0, v1)
unpack <- function(theta) list(
  ...
  v0 = theta[8], v1 = theta[9])
psi_of <- function(P, age) exp(P$v0 + P$v1 * log1p(age))
...
cell_ll <- function(z, mu, aa, bb, post, P) {
  ...
  if (any(on)) out[on] <- lse3(
    log1p(-pi_[on]) + l0[on],
    log(pi_[on]) + log1p(-P$q) + lskel(z[on], alpha[on] + psi[on] * lamS[on], lamS[on]),
    log(pi_[on]) + log(P$q)    + lskel(z[on], alpha[on] + psi[on] * lamL[on], lamL[on]))
  out
}
...
# seeded at the script 30 optimum with the asymmetry opened up
WARM <- c(-3.50, 0.50, 2.26, -2.00, stats::qlogis(0.411), log(15.64), log(5.7555), 0.15, -0.10)
starts <- if (nzchar(Sys.getenv("COLD"))) list(
               WARM,
               c(-3.50, 0.50, 2.26, -2.00, stats::qlogis(0.411), log(15.64), log(5.7555), 0.4, -0.3),
               c(-3.50, 0.50, 2.00, -1.80, stats::qlogis(0.30), log(10), log(4.0), 0, 0)) else list(WARM)
...
CKPT <- sprintf("devel/spa_diagnostics/psiagefit_%s.rds", gsub(" ", "", STATE))
...
cat(sprintf("scales : q = %.3f of revisions are LARGE | sigma_s = %.2f (additive) | sigma_1 = %.4f x level\n",
            P$q, P$ss, P$s1))
cat(sprintf("psi(a) : exp(%.3f %+.3f log(1+a)) -> psi(1) = %.3f, psi(4) = %.3f, psi(15) = %.3f\n",
            P$v0, P$v1, psi_of(P, 1), psi_of(P, 4), psi_of(P, 15)))
cat("         (>1 tilts the revision upward, 1 = symmetric)\n\n")
...
pmf <- function(z, i) {                # z may be a vector; i is a single cell
  ...
  exp(lse3(log1p(-pi_[i]) + lpois(z, alpha[i]),
           log(pi_[i]) + log1p(-P$q) + lskel(z, alpha[i] + psi_i[i] * lamS[i], lamS[i]),
           log(pi_[i]) + log(P$q)    + lskel(z, alpha[i] + psi_i[i] * lamL[i], lamL[i])))
}
k <- which(fl$post)
...
cdf_at <- function(i, zq) {
  sd_i <- sqrt(alpha[i] + (1 + psi_i[i]) * max(lamS[i], lamL[i]))
...
cat(sprintf("psi(a) two-scale (9 par)      : %.2f\n", -best$value))
...
saveRDS(list(state = STATE, par = best$par, logL = -best$value, mu = mu,
        ...
        sprintf("devel/spa_diagnostics/psiage_%s.rds", gsub(" ", "", STATE)))
