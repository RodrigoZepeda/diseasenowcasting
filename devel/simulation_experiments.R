...
test_nowcast <- NobBS(data=denguedat, units="1 week", now = now,
                      true_date="onset_week", report_date="report_week")
...
ggplot() +
  ...
  geom_ribbon(aes(x = true_date, ymin = lower, ymax = upper, fill = "NobBS"),
  ...
  geom_line(aes(x = true_date, y = estimate, color = "NobBS"), data = test_nowcast$estimates) +
