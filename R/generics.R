...
#'
#' @param metrics the metrics to display. Default = "wis",
#'  accepts multiple metrics
#'
...
#' it also returns the metrics across all the horizons ("all" in column "horizon")
#'
#' @examples
#' #' These examples require the `scoringutils` package
#' if (requireNamespace("scoringutils", quietly = TRUE)) {
...
#'
#'   # Create a summary
#'   #summary(comparison)                    ADJ AFTER FIX
#'
#'   # Create a summary with more metrics:
#'   #summary(comparison, metrics = c("wis","se_point","ape","mse", "ae_median")) #ADJ AFTER FIX (new names)
#'   @export

summary.backtest_metrics <- function(x, ..., metrics = "wis") {
  ...
  horizons=x$horizon # uses all horizon available
  ...
  metric_summary <- x |>
    ...
    dplyr::bind_rows(  # Add overall averages
      x |>
        dplyr::filter(horizon %in% horizons) |>
        tidyr::pivot_longer(
          cols = dplyr::all_of(metrics),
