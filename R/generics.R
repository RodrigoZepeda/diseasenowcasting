...
#' @param ... Can be used to specify which quantiles are desired (e.g. `quantiles = c(0.2, 0.8)`).
...
#' summary(ncast, quantiles = c(0.2, 0.4))
#'
#' @export

summary.nowcaster <- function(object, ...) {

  if (is.null(list(...)[["quantiles"]])){
...
#'
#' @section Personalize your plot:
#'
#' The following parameters can be used in the `...` section
#'
#' + `color` A string indicating the color for the barplots. Works with the default R colors of
#' [grDevices::colors()]. Hex color codes works as well. Best results with darker colors.
#'
#' + `datesbrakes` A string giving the distance between x-axis breaks if not specified, one label
#' per bar. Other valid examples are: "2 weeks", or "10 years", 'sec', 'min', 'hour', 'day', 'week',
#' 'month', 'year', optionally followed by 's'.
#'
#' + `casesbrakes` Desired number of y-axis breaks. You may get slightly more or fewer breaks
#' that requested due to dynamic adaptation.
#'
#' + `rowsfacet` Number of rows for arranging facets when using [ggplot2::facet_wrap()]. It allows
#' manual control over the layout of multiple strata in the plot.
#'
#' + `colsfacet` Number of columns for arranging facets when using [ggplot2::facet_wrap()]. It
#' allows manual control over the layout of multiple strata in the plot.
#'
#' + `quantiles` a vector of two values to specify the quantiles for the error bars.
#'
...
plot.nowcaster <- function(x, ...){
  ...
  if (!exists("datesbrakes")){
    datesbrakes <- NULL
  }

  if (!exists("casesbrakes")){
    casesbrakes <- 10
  }
