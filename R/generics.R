...
#'
#' @param object A backtest_metrics object generated by [backtest_metrics()]
#'
#' @param ... further arguments passed to the generic [summary()] function.
#'
#' @param metrics selects the metrics to display.
...
#' @export
#'
summary.backtest_metrics <- function(object, ..., metrics = "all") {

  available_metrics <- colnames(object)[-c(1:4)]
  ...
  if (any(!metrics %in% available_metrics)) {
    cli::cli_abort("At least one metric you selected was not evaluated in backtest_metrics() \n
                   Available metrics to summarize are: {.strong {paste(available_metrics, collapse = ', ')}}")
  }

  horizons <- unique(object$horizon)  # Uses all available horizons
  ...
  metric_summary <- object |>
    ...
    dplyr::mutate(horizon = as.character(.data$horizon)) |>  # Ensure horizon is character
    dplyr::group_by(
      .data$model,
      .data$Strata_unified,
      .data$horizon,
    ...
    dplyr::summarize(
      ...
      n_now = sum(!is.na(.data$value)),  # Number of non-missing values
      se = .data$sd / sqrt(.data$n_now),  # Standard error of the mean
      .groups = "drop"
    )

  # Add rows with combined horizons
  if (length(horizons) != 1) {
    overall_summary <- object |>
      ...
      dplyr::summarize(
        ...
        avg = mean(.data$value, na.rm = TRUE),
        sd = sd(.data$value, na.rm = TRUE),
        n_now = sum(!is.na(.data$value)),
        se = .data$sd / sqrt(.data$n_now),
        .groups = "drop"
      ) |>
      dplyr::mutate(horizon = as.character(.data$horizon))  # Ensure horizon is character
