...
summary.nowcaster <- function(nowcast_output, quantiles=c(0.025, 0.975)) {
  ...
  true_date_name <- nowcast_output$data$call_parameters$true_date
...
#' @export
plot.nowcaster <- function(nowcast_output,
                         color = "deepskyblue4",
                         datesbrakes = NULL,
                         casesbrakes = 10,
                         rowsfacet = NULL,
...
#' ncast <- nowcast(first_1k, "onset_week", "report_week", now = now,
