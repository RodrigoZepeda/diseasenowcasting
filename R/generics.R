...
plot.backtest_metrics <- function(x, ..., metric = "wis", horizons = 0, datesbrakes = NULL) {
  ...
  # if there is only one metric in x , wis is overwritten and it plots that one
  if (length(available_metrics) == 1 && available_metrics != metric) {
...
#'
#' @param x A backtest_metrics object generated by [backtest_metrics()]
#'
#' @param metrics the metrics to display.
#' metrics = "all" summarizes all available metrics generated in [backtest_metrics()]
#'
#' @param ... further arguments passed to the generic [summary()] function.
#'
#' @return A summary tibble of the [backtest_metrics()] results,
#' it specifies: average, standard deviation, standard error, number of "now" dates
#' it also returns the metrics across all the horizons provided ("all" in column "horizon")
#'
#' @examples
#' # These examples require the `scoringutils` package
#' if (requireNamespace("scoringutils", quietly = TRUE)) {
...
summary.backtest_metrics <- function(x, metrics = "all",... ) {

  available_metrics <- colnames(x)[-c(1:4)]

  #if null takes all metrics
  ...
  if (any(!metrics %in% available_metrics)) {
    cli::cli_abort("At least a metric you selected was not evaluated in backtest_metrics() \n
                   Available metrics to summarize are: {.strong {paste(available_metrics, collapse = ', ')}}")
  }

  horizons=unique(x$horizon) # uses all horizon available
  ...
  metric_summary <- x |>
    ...
    dplyr::mutate(horizon = as.character(!!rlang::sym("horizon"))) |>  # Ensure horizon is character
    dplyr::group_by(
      !!rlang::sym("model"),
      !!rlang::sym("Strata_unified"),
      !!rlang::sym("horizon"),
    ...
    dplyr::summarize(
      avg = mean(!!rlang::sym("value"), na.rm = TRUE),
      sd = sd(!!rlang::sym("value"), na.rm = TRUE),
      n_now = sum(!is.na(!!rlang::sym("value"))),  # Number of non-missing values
      se = sd / sqrt(n_now),  # Standard error of the mean
      .groups = "drop"
    )
  #adds the lines with the horizon combined
  if (length(horizons) != 1) {
    overall_summary <- x |>
      ...
      dplyr::summarize(
        horizon = "all",  # Label for combined horizons
        avg = mean(!!rlang::sym("value"), na.rm = TRUE),
        sd = sd(!!rlang::sym("value"), na.rm = TRUE),
        n_now = sum(!is.na(!!rlang::sym("value"))),
        se = sd / sqrt(n_now),
        .groups = "drop"
      ) |>
      dplyr::mutate(horizon = as.character(!!rlang::sym("horizon")))  # Ensure horizon is character in new rows

    metric_summary <- dplyr::bind_rows(metric_summary, overall_summary)
  }
